        ...
        for tmp in now_list:
            ...
            for counter, (tar, src) in enumerate(all_images):
                ...
                if src[0] is None:  # do not manipulate
                    ...
                    im_mask[im_mask >= im_mask.max()/2] = 1.
                    im_mask[im_mask <= im_mask.max()/2] = 0
                    if im_mask is not None and not np.any(im_mask > 0):
                        src = (None, None)
                        im_mask, im_mask_new = None, None
                    if im_s.shape[:2] != im_mask.shape[:2]:
                        im_mask = cv2.resize(im_mask, (im_s.shape[1], im_s.shape[0]), interpolation=0)
                if src[0] is not None:
                        ...
                        if max_scale < 0.75:
                            ...
                            if prev_scale == -1:
                                ...
                                scale = np.random.choice(
                                    np.linspace(0.75, min(1, max_scale), 30)
                                )
                            else:
                                scale = prev_scale
                                if scale > max_scale:
                    ...
                    if translate is not None:
                        ...
                        im_mask_new, flag = utils.patch_transform_s(
                            im_mask, mask_orig_bb, centroid_orig, translate, scale, order=0
                        )
                        ...
                        im_s_n, _ = utils.patch_transform_s(
                            im_s_masked, mask_orig_bb, centroid_orig, translate, scale, order=0
                        )
